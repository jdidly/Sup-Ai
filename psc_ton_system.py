...
class PSCTONTradingBot:
    ...
    def __init__(self):
        ...
        self.min_signal_ratio = 1.25  # PSC threshold: Realized/Implied >= 1.25
        ...
        self.trades_log_file = Path("data/live_trades.csv")
        self.signals_log_file = Path("data/psc_signals.csv")
        self.daily_summary_file = Path("data/daily_summaries.csv")
        ...
        try:
            from src.paper_trading_validator import PaperTradingValidator
    ...
    def create_superp_position(self, asset: str, price: float, psc_ratio: float, 
        ...
        try:
            ...
            current_timer_minute = datetime.now().minute % 10
    ...
    def log_trade(self, coin, entry_price, exit_price, confidence, ml_prediction, ratio, direction, successful, profit_pct=0, profit_usd=0):
        ...
        try:
            ...
            logger.info(f"📊 Trade logged: {coin} - {direction} - Profit: {profit_pct:.2f}%")
...
Or edit config/settings.yaml directly
    ...
    async def performance_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            # Load trading data
            trade_file = Path("data/live_trades.csv")
...
*Professional AI-Powered Bidirectional Trading*
    ...
    async def status_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        self.timer_minute = current_time.minute % 10
...
🧪 *Paper trading validates every prediction for ML improvement*
    ...
    def determine_trade_direction(self, crypto, ratio, confidence):
        """Determine trade direction based on PSC arbitrage: Realized/Implied ratio analysis"""
        # PSC Logic: Higher ratios indicate realized volatility > implied volatility = arbitrage opportunity
        # LOWERED THRESHOLDS FOR DATA GATHERING
        if ratio >= 2.0 and confidence > 0.5:  # Lowered from 0.7
            return "LONG", "📈", "Strong PSC arbitrage - high realized/implied ratio"
        elif ratio >= 1.5 and confidence > 0.4:  # Lowered from 0.6
            return "LONG", "📈", "Good PSC opportunity - moderate ratio advantage"
        elif ratio >= 1.25 and confidence > 0.35:  # Lowered from 0.5
            return "LONG", "📈", "Entry-level PSC arbitrage signal"
        elif ratio < 1.0 and confidence > 0.4:  # Lowered from 0.6
            return "SHORT", "📉", "Implied > Realized - potential correction"
        elif ratio <= 0.8 and confidence > 0.5:  # Lowered from 0.7 - Strong short signal
            return "SHORT", "📉", "Strong downward pressure - high short confidence"
        elif ratio <= 0.9 and confidence > 0.35:  # Lowered from 0.5 - Moderate short signal
            return "SHORT", "📉", "Moderate short opportunity - ratio below threshold"
        else:
            return "NEUTRAL", "↔️", "Ratio borderline - mixed PSC signals"
        ...
        while self.running:
            try:
                ...
                if self.timer_minute < 3 and random.random() < 0.15:  # 15% chance during entry window
                    ...
                    base_ratio = round(current_price / (ton_price * 0.001), 2)  # Adjusted for different scales
